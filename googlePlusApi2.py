...
if __name__=='__main__':
    coname_twitter_account_file = pd.read_excel('twitterHandles.xlsx', sheetname='Sheet1')
    #with open('PinterestData.csv','a',newline='',encoding='utf-8') as file:
        ...
        try:
            if pd.isnull(twitterMainHandle):
                try:
                    ...
                    #csvRowDataArr.append(coname)
                    #csvRowDataArr.append(twitterUserMainHandle.replace('@','').lower())
                    #writer.writerow(csvRowDataArr)
                    #csvRowDataArr=[]
                except:
                    pass
            else:
                ...
                try:
                    ...
                    df = pd.DataFrame({'Posts': titleArr,'Published Date':publishedDateArr,'PlusOners':plusonersArr,'Replies':repliesArr,'Resharers':resharersArr})
                    df.to_excel(writer, sheet_name=url_part+'Posts')
                    writer.save()
